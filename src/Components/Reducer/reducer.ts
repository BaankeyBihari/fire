...
export function compareInvestment(a: Investment, b: Investment) {
    ...
    if (a.tag === 'Planned') { return 1 }
    if (b.tag === 'Planned') { return -1 }
...
function generatePlan(
    ...
    let limitDate = addYears(startDate, 50);
    let investmentPlan: Investment[] = [];
    let dailyInflation = expectedAnnualInflation / 36500;
    let dailyGrowthRate = expectedGrowthRate / 36500;
    let dailySIPRate = sipGrowthRate / 36500;
    ...
    while (currentAmount * delta * dailyGrowthRate < currentTarget && nextSIPDate < limitDate) {
        let currentInvestmentRecord = new DummyInvestment();
        ...
        currentAmount = currentAmount * delta * dailyGrowthRate + currentSIP + currentAmount;
